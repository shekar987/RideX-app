...
function useAdminStats() {
  ...
  useEffect(() => {
    ...
    // Bounded by today's date (createdAt >= midnight) — never grows past one
    // day of rides, so no limit() needed here.
...
function useDrivers() {
  ...
  // re-sort newest-first to keep the original display order. createdAt may be
  // a Firestore Timestamp (REST-written docs), a number, or an ISO string.
  const toMs = t => t?.toMillis?.() ?? (typeof t === 'number' ? t : new Date(t || 0).getTime() || 0);
  const drivers = (pendingDrivers === null || liveDrivers === null)
    ? null
    : dedupeById([...pendingDrivers, ...liveDrivers, ...olderDrivers])
        .sort((a, b) => toMs(b.createdAt) - toMs(a.createdAt));

  return { drivers, hasMore, loadingMore, loadMore };
}

// ── Drivers section ───────────────────────────────────────────────────────────
function DriversSection() {
  const { drivers, hasMore, loadingMore, loadMore } = useDrivers();
  ...
  // exact (dedicated live query). Accurate totals need server-side aggregation
  // (getCountFromServer with where()) or a stats document — future task.
...
// pattern as HistoryTab in DriverDashboard.jsx.
function useAllRides() {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const snap = await getDocs(q);
      olderLoadedRef.current = true;
      setOlderRides(prev => [...prev, ...snap.docs.map(d => ({ id: d.id, ...d.data() }))]);
      setLastDoc(snap.docs[snap.docs.length - 1] || null);
      setHasMore(snap.docs.length === PAGE_SIZE);
    } catch (_) {}
    setLoadingMore(false);
  ...
  const rides = liveRides === null ? null : dedupeById([...liveRides, ...olderRides]);
  return { rides, totalCount, hasMore, loadingMore, loadMore };
}
...
function RidesSection() {
  const { rides, totalCount, hasMore, loadingMore, loadMore } = useAllRides();
  const [filter, setFilter] = useState('all');
  ...
  // Pills no longer show per-status counts: the query is limited to loaded
  // pages, so client-side counts would undercount. Accurate per-status counts
  // need server-side aggregation (getCountFromServer with a where() filter)
  // or a maintained stats document — future task.
  const FILTERS = [
    { id: 'all',         label: 'All'         },
